...
- 表示スロットは「イベント開始位置＋空き領域の 16 分単位位置」で構成し、イベントスロットの幅は音価に対して劣線形（`len^0.62`）に比例させる（音価比例スペーシング）。イベント持続中の tick は独立したスロットを持たず、イベント開始スロットがその領域全体を占める。ノート・休符・フレット数字はスロット中央ではなく**スロット左端から基準幅の半分**の位置にアンカーする(全音符が小節中央に来ない)
